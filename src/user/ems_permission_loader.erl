       ...
       case ems_odbc_pool:get_connection(Datasource) of
          {ok, Datasource2} ->
              Result = case ems_odbc_pool:param_query(Datasource2,
              			...
              			{_,_, Records} ->
              			    F = fun() ->
              			       insert_permissions(Records, 0, CtrlInsert)
              		    end,
              		    mnesia:ets(F),
          ...
          {ok, Datasource2} ->
               case LastUpdate of
				  ...
				  Result = case ems_odbc_pool:param_query(Datasource2, Sql, Params, ?MAX_TIME_ODBC_QUERY) of
				  	  ...
				  	  {_,_,Records} ->
				  	       F = fun() ->
				  	           update_permissions(Records, 0, CtrlUpdate)
				  	       end,
...
sql_load_permissions() ->
  " select distinct u.UsuId as codigo, u.UsuLogin as login, 
        p.PerId as perId, p.PerNome as perNome, t.TraId as traId, 
        t.TraNomeFrm as traNameFrm, t.TraNomeMenu as traNameMenu,
		t.TraIncluir as traInclude, t.TraAlterar as traUpdate, 
		t.TraExcluir as traExclude, t.TraVisualizar as traVisualize
	    from TB_Usuario u  inner join TB_Acessos_Perfil up  
		on u.UsuId = up.APeUsuId inner join TB_Perfil p 
		on up.APePerId = p.PerId inner join TB_Sistemas s 
	    on p.PerSisId = s.SisId inner join TB_Perfil_Transacao pt 
	    on p.PerId = pt.PTrPerId inner join TB_Transacao t 
