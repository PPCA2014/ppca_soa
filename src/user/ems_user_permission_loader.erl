...
-export([init/1, handle_call/3, handle_cast/2, handle_info/1, handle_info/2, terminate/2, code_change/3, 
		 last_update/0, is_empty/0, size_table/0, force_load_permissions/0]).
...
init(#service{datasource = Datasource, 
	...
	set_force_load_permissions_checkpoint(),
...
handle_cast(force_load_permissions, State = #state{update_checkpoint = UpdateCheckpoint}) ->
	ems_logger:warn("ems_user_permission_loader force load the permissions now."),
...
handle_info(check_force_load_permissions, State = #state{update_checkpoint = UpdateCheckpoint}) ->
	{{_, _, _}, {Hour, _, _}} = calendar:local_time(),
	case Hour >= 4 andalso Hour =< 6 of
		true ->
			ems_logger:info("ems_user_permission_loader force load permissions checkpoint."),
			State2 = State#state{last_update = undefined},
			case update_or_load_permissions(State2) of
				{ok, State3} ->
					erlang:send_after(86400 * 1000, self(), check_force_load_permissions),
					{noreply, State3, UpdateCheckpoint};
				{error, State3} -> 
					erlang:send_after(60000 * 5, self(), check_force_load_permissions),
					{noreply, State3, UpdateCheckpoint}
			end;
		_ -> 
			set_force_load_permissions_checkpoint(),
...
set_force_load_permissions_checkpoint() ->
	erlang:send_after(60000 * 60, self(), check_force_load_permissions).
