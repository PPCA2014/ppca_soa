...
new_from_map(Map, _Conf, Id) ->
	try
		{ok, #user_dados_funcionais{
					...
					lotacao = ?UTF8_STRING(maps:get(<<"lotacao">>, Map, <<>>)),
					lotacao_sigla = ?UTF8_STRING(maps:get(<<"lotacao_sigla">>, Map, <<>>)),
					lotacao_centro = ?UTF8_STRING(maps:get(<<"lotacao_centro">>, Map, <<>>)),
					lotacao_codigo_funcao = maps:get(<<"lotacao_codigo_funcao">>, Map, <<>>),
					lotacao_funcao = ?UTF8_STRING(maps:get(<<"lotacao_funcao">>, Map, <<>>)),
					lotacao_orgao = ?UTF8_STRING(maps:get(<<"lotacao_orgao">>, Map, <<>>)),
					lotacao_codigo_cargo = maps:get(<<"lotacao_codigo_cargo">>, Map, <<>>),
					lotacao_cargo = ?UTF8_STRING(maps:get(<<"lotacao_cargo">>, Map, <<>>)),
