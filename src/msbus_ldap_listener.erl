%%********************************************************************
%% @title Module msbus_ldap_listener
%% @version 1.0.0
...
init({Port, IpAddress}) ->
	...
	case gen_tcp:listen(Port, Opts) of
		{ok, LSocket} ->
				msbus_logger:debug("Start ldap workers for listener ~p, IP ~p.", [self(), IpAddress]),    
				start_server_workers(Conf#config.tcp_max_http_worker, 
									 LSocket,
									 Conf#config.tcp_allowed_address_t),
				msbus_logger:info("Listening ldap packets on ~s:~p.", [inet:ntoa(IpAddress), Port]),
...
start_server_workers(Num, LSocket, Allowed_Address) ->
    msbus_ldap_worker:start_link({Num, LSocket, Allowed_Address}),
